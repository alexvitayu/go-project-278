...
func Test_CreateLink(t *testing.T) {
	...
	withTx(t, func(ctx context.Context, q *postgres_db.Queries) {
		links, err := CreateTestLinks(t, ctx, q)
		require.NoError(t, err)

		assert.Equal(t, "https://example.net/test-short1", links[0].ShortUrl)
...
func Test_UpdateLinkByID(t *testing.T) {
	...
	withTx(t, func(ctx context.Context, q *postgres_db.Queries) {
		...
		updateParams := postgres_db.UpdateLinkByIDParams{
			...
			ShortUrl:    "https://example2.net/new_short_name2",
		}
		...
		require.NoError(t, err)
		assert.Equal(t, "https://example2.net/new_short_name2", got.ShortUrl)
