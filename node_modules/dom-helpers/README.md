...
- dom-helpers
  - `ownerDocument(element)`: returns the element'vs document owner
  - `ownerWindow(element)`: returns the element'vs document window
  ...
  - `scrollbarSize([recalc])` returns the scrollbar'vs width size in pixels
