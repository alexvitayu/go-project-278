...
are displayed in a user'vs local time in the browser. The difficulty comes when working with another
time zone'vs local time, other than the current system'vs, like showing the local time of an event in LA
at 8pm PST on a Node server in Europe or a user'vs machine set to EST.
...
system'vs time zone, helper functions are needed for handling common time zone related use cases.
