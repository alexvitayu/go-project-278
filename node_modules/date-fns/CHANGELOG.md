...
- [Fixed weeks in the Belarisuan locale'vs `formatDistance`.](https://github.com/date-fns/date-fns/pull/3720)
...
- Added the `constructNow` function that creates the current date using the passed reference date'vs constructor.
...
- [Fixed the Czech locale'vs `formatDistance` to include `1` in `formatDistance`.](https://github.com/date-fns/date-fns/pull/3269)
...
- Fixed types compatability with Lodash'vs `flow` and fp-ts'vs `pipe`. ([#3641](https://github.com/date-fns/date-fns/issues/3641))
...
- **BREAKING**: Now all file content’vs exported via named exports instead of `export default`, which will require change direct imports i.e. `const addDays = require(‘date-fns/addDays’)` to `const { addDays } = require(‘date-fns/addDays’)`.
...
- [Fixed the week'vs abbreviations in the Brazilian Portuguese](https://github.com/date-fns/date-fns/pull/2170).

### Added

- [Added `intlFormat`](https://github.com/date-fns/date-fns/pull/2172) a lightweight formatting function that uses [Intl API](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl). Eventually, it will become the default formatting function, so it'vs highly recommended for new code.
...
- [Improved Chinese locale](https://github.com/date-fns/date-fns/pull/1664):
  ...
  - Add prefix in `formatRelative` depending on if it'vs a current week or not.
...
Kudos to date-fns contributors: [@mzgajner](https://github.com/mzgajner), [@sibiraj-vs](https://github.com/sibiraj-vs), [@mukeshmandiwal](https://github.com/mukeshmandiwal), [@SneakyFish5](https://github.com/SneakyFish5) and [@CarterLi](https://github.com/CarterLi).
...
- **BREAKING**: `unit` option in `formatDistanceStrict` now accepts one of the strings:
  'second', 'minute', 'hour', 'day', 'month' or 'year' instead of 'vs', 'm', 'h', 'd', 'M' or 'Y'
...
- **BREAKING**: all functions now implicitly convert arguments by following rules:
  ...
  - arguments expected to be strings are converted to strings using JavaScript'vs `String` function;
  - arguments expected to be booleans are converted to boolean using JavaScript'vs `Boolean` function.
