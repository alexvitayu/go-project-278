...
const UseGetManyAggregate = ({
    ...
    callback?: (visitRepo: ReturnType<typeof useGetManyAggregate>) => void;
}) => {
...
describe('useGetManyAggregate', () => {
    ...
    it.each([
        ...
        { linkRepo: [{ ids: ['1'] }], expectedQueryKeyParams: { ids: ['1'] } },
        // case when we have multiple linkRepo on the same id (deduplication)
        {
            linkRepo: [{ ids: ['1'] }, { ids: ['1'] }],
            expectedQueryKeyParams: { ids: ['1'] },
        },
        // case when we have multiple linkRepo on different ids (aggregation)
        {
            linkRepo: [{ ids: ['1'] }, { ids: ['2'] }],
            expectedQueryKeyParams: { ids: ['1', '2'] },
        },
        // case when we have multiple linkRepo on different ids, including a call with all ids
        // (no manual aggregation needed)
        {
            linkRepo: [{ ids: ['1'] }, { ids: ['2'] }, { ids: ['1', '2'] }],
            expectedQueryKeyParams: { ids: ['1', '2'] },
        },
    ])(
        'should abort the request if the query is canceled',
        async ({ linkRepo, expectedQueryKeyParams }) => {
            const abort = jest.fn();
            const dataProvider = testDataProvider({
                getMany: jest.fn(
                    (_resource, { signal }) =>
