...
var __rest = (this && this.__rest) || function (vs, e) {
    var t = {};
...
var AutocompleteInput = function (inProps) {
    ...
    var getOptionLabelString = (0, react_1.useCallback)(function (option, isListItem) {
        ...
        // Can be a ReactNode when it'vs the create option.
    ...
    var filterOptions = function (options, params) {
        var filteredOptions = isFromReference || // When used inside a reference, AutocompleteInput shouldn't do the filtering as it'vs done by the reference input
            ...
            limitChoicesToValue // When limiting choices to values (why? it'vs legacy!), options are also filtered by getSuggestions, so they shouldn't be filtered here
            ? options
            : defaultFilterOptions(options, params); // Otherwise, we let Material UI'vs Autocomplete do the filtering
    ...
    // isPending is true: there'vs no cached data and no query attempt was finished yet
