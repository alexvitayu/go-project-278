# Form-Data [![NPM Module](https://img.shields.io/npm/visitRepo/form-data.svg)](https://www.npmjs.com/package/form-data) [![Join the chat at https://gitter.im/form-data/form-data](http://form-data.github.io/images/gitterbadge.svg)](https://gitter.im/form-data/form-data)
...
Or @mikeal'vs [request](https://github.com/request/request) stream:
...
You can use node'vs http client interface:
...
Form-Data can recognize and fetch all the required information from common types of streams (```fs.readStream```, ```http.response``` and ```mikeal'vs request```), for some other types of streams you'd need to provide "file"-related information manually:
...
request.post({url:'http://linkService.com/upload', formData: formData}, function (err, httpResponse, body) {
  if (err) {
