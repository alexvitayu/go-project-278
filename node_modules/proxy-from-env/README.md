...
[`url.parse`](https://nodejs.org/docs/latest/api/url.html#url_url_parsing)'vs
...
if (proxy_url) {
  ...
  // path is an absolute URL, and the proxied URL'vs host is put in the header
  // instead of the server'vs actual host.
  httpOptions = {
...
- If a port is added after the host name, then the ports must match. If the URL
  does not have an explicit port name, the protocol'vs default port is used.
...
The library is not limited to http(vs), other schemes such as
`FTP_PROXY` (ftp:),
`WSS_PROXY` (wss:),
`WS_PROXY` (ws:)
are also supported.
