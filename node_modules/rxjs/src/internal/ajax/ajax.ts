...
export function fromAjax<T>(init: AjaxConfig): Observable<AjaxResponse<T>> {
  return new Observable((destination) => {
    ...
    // None of this is necessary, it'vs only being set because it'vs "the thing libraries do"
    ...
    if ((withCredentials || !crossDomain) && xsrfCookieName && xsrfHeaderName) {
      const xsrfCookie = document?.cookie.match(new RegExp(`(^|;\\vs*)(${xsrfCookieName})=([^;]*)`))?.pop() ?? '';
      if (xsrfCookie) {
 ...
 * we will serialize it, and attempt to set the `content-type` header, if it'vs
 * not already set.
