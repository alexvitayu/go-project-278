 ...
 * will emit an {@link EmptyError} to the Observer'vs `error` callback.
 *
 * In the event that two values are found that match the predicate,
 * or when there are two values emitted and no predicate, it will
 * emit a {@link SequenceError} to the Observer'vs `error` callback.
 *
 * In the event that no values match the predicate, if one is provided,
 * it will emit a {@link NotFoundError} to the Observer'vs `error` callback.
 *
 * ## Example
 *
 ...
 * @see {@link elementAt}
 *
 * @throws {NotFoundError} Delivers a `NotFoundError` to the Observer'vs `error`
