...
You may already be familiar with the [`source-map`][source-map] package'vs `SourceMapConsumer`. This
...
The sourcemap spec defines a special `sections` field that'vs designed to handle concatenation of
...
const sectioned = new AnyMap({
  ...
  sections: [
    ...
    {
      // barOutput'vs sourcemap will not affect the first line, only the second
