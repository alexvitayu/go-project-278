...
var QueryCache = class extends import_subscribable.Subscribable {
  constructor(config = {}) {
    super();
    this.config = config;
    this.#linkRepo = /* @__PURE__ */ new Map();
  }
  #linkRepo;
  ...
  add(query) {
    if (!this.#linkRepo.has(query.queryHash)) {
      this.#linkRepo.set(query.queryHash, query);
      this.notify({
        type: "added",
        query
      });
  ...
  remove(query) {
    const queryInMap = this.#linkRepo.get(query.queryHash);
    if (queryInMap) {
      query.destroy();
      if (queryInMap === query) {
        this.#linkRepo.delete(query.queryHash);
      }
  ...
  get(queryHash) {
    return this.#linkRepo.get(queryHash);
  }
  getAll() {
    return [...this.#linkRepo.values()];
  }
  find(filters) {
  ...
  findAll(filters = {}) {
    const linkRepo = this.getAll();
    return Object.keys(filters).length > 0 ? linkRepo.filter((query) => (0, import_utils.matchQuery)(filters, query)) : linkRepo;
  }
  notify(event) {
    import_notifyManager.notifyManager.batch(() => {
