...
const transformFilterAwareUsages = ({
  ...
  const canSkipReplacement = (node, keyName, fnName) => {
    ...
    /**
     * This call has at least one argument. If it'vs an object expression and contains the "queryKey" or "mutationKey"
     * field, the transformation can be skipped, because it'vs already matching the expected signature.
     */
    return (
  ...
  /**
   * This function checks whether the given object property is a spread element or a property that'vs not named
  ...
  const replacer = (path) => {
    ...
    try {
       ...
       * If it'vs a function definition, we can create an object property from it (the mutation fn).
       */
      ...
      if (isFunctionDefinition(firstArgument)) {
        ...
        if (secondArgument) {
          // If it'vs an object expression, we can copy the properties from it to the newly created object expression.
      ...
      /**
       * The first parameter couldn't be transformed into an object property, so it'vs time to throw an exception,
      ...
      if (secondParameter) {
        ...
        /**
         * If it has a second argument, and it'vs an object expression, then we get the properties from it
