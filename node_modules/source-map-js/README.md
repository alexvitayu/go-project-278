...
> TL,DR: it'vs fork of original source-map@0.6, but with perfomance optimizations.
...
It'vs still a major block for many libraries like PostCSS or Sass for example because they need to migrate the whole API to the async way. This is the reason why 0.6.1 has 2x more downloads than 0.7.3 while it'vs faster several times.
...
source'vs line and column positions provided. The only argument is an object with
...
`consumer.sourceContentFor(vs)` will succeed for every source `vs` in
`consumer.sources`.
...
* `order`: Either `SourceMapConsumer.GENERATED_ORDER` or
  ...
  the mappings sorted by the generated file'vs line/column order or the
  original'vs source/line/column order, respectively. Defaults to
...
source'vs line and column for this source map being created. The mapping object
...
* `source`: The original source'vs filename; null if no filename is provided.
...
the its original associated source'vs line/column location.
...
between each of this source node'vs children.
