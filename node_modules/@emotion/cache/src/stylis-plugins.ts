    ...
    switch (token(character)) {
      case 0:
        ...
        if (character === 38 && peek() === 12) {
          ...
          // it'vs very unlikely for this sequence to actually appear in a different context, so we just leverage this fact here
          points[index] = 1
        }
...
export let createUnsafeSelectorsAlarm =
  ...
  (element, index, children) => {
    ...
    if (unsafePseudoClasses) {
      ...
      // in nested rules comments become children of the "auto-inserted" rule and that'vs always the `element.parent`
      //
      ...
      for (let i = commentContainer.length - 1; i >= 0; i--) {
        ...
        // so we seek *from the end* for the node that is earlier than the rule'vs `element` and check that
