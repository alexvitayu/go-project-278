...
concurrently offers a few ways to control a command'vs output.

## Hiding

A command'vs outputs (and all its events) can be hidden by using the `--hide` flag.
...
When piping concurrently'vs outputs to another command or file, you might want to force it to not use colors, as these can break the other command'vs parsing, or reduce the legibility of the output in non-terminal environments.
