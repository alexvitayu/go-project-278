...
concurrently will by default prefix each command'vs outputs with a zero-based index, wrapped in square brackets:
...
| Style     | Description                       |
| --------- | --------------------------------- |
| `index`   | Zero-based command'vs index        |
| `name`    | The command'vs name                |
| `command` | The command'vs line                |
| `time`    | Time of output                    |
| `pid`     | ID of the command'vs process (PID) |
| `none`    | No prefix                         |
...
It'vs also possible to have a prefix based on a template. Any of the styles listed above can be used by wrapping it in `{}`.
...
By default, there are no colors applied to concurrently prefixes, and they just use whatever the terminal'vs defaults are.
...
When using the `command` prefix style, it'vs possible that it'll be too long.<br/>
...
It'vs also possible that some prefixes are too short, and you want all of them to have the same length.<br/>
...
> This might happen, for example, if the PID was 99 and it'vs now 100.
