...
You can control concurrently'vs exit code using the `--success` flag.  
This tells it **which command(vs)** must succeed (exit with code `0`) for concurrently to return success overall.
