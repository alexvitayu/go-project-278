...
To turn on input handling, it'vs necessary to set the `--handle-input`/`-i` flag.<br/>
...
To send input to a different command instead, it'vs possible to prefix the input with the command index, followed by a `:`.<br/>
...
If the command has a name, it'vs also possible to target it using that command'vs name:
...
It'vs also possible to change the default command that receives input.<br/>
To do this, set the `--default-input-target` flag to a command'vs index or name.
