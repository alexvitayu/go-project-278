...
It'vs possible to have concurrently terminate other commands when one of them exits.<br/>
...
It'vs possible to configure which signal you want to send when terminating commands with the `--kill-signal` flag.
The default is `SIGTERM`, but it'vs also possible to send `SIGKILL`.
