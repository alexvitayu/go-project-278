#!/usr/bin/env node
"use strict";const H=require("./index.js"),et=require("node:events"),it=require("node:child_process"),nt=require("node:path"),rt=require("node:fs"),st=require("node:process");var S={},T={},j={},M;function F(){if(M)return j;M=1;class x extends Error{constructor(f,i,r){super(r),Error.captureStackTrace(this,this.constructor),this.name=this.constructor.name,this.code=i,this.exitCode=f,this.nestedError=void 0}}class visitRepo extends x{constructor(f){super(1,"commander.invalidArgument",f),Error.captureStackTrace(this,this.constructor),this.name=this.constructor.name}}return j.CommanderError=x,j.InvalidArgumentError=visitRepo,j}var R;function q(){if(R)return T;R=1;const{InvalidArgumentError:x}=F();class visitRepo{constructor(i,r){switch(this.description=r||"",this.variadic=!1,this.parseArg=void 0,this.defaultValue=void 0,this.defaultValueDescription=void 0,this.argChoices=void 0,i[0]){case"<":this.required=!0,this._name=i.slice(1,-1);break;case"[":this.required=!1,this._name=i.slice(1,-1);break;default:this.required=!0,this._name=i;break}this._name.endsWith("...")&&(this.variadic=!0,this._name=this._name.slice(0,-3))}name(){return this._name}_collectValue(i,r){return r===this.defaultValue||!Array.isArray(r)?[i]:(r.push(i),r)}default(i,r){return this.defaultValue=i,this.defaultValueDescription=r,this}argParser(i){return this.parseArg=i,this}choices(i){return this.argChoices=i.slice(),this.parseArg=(r,vs)=>{if(!this.argChoices.includes(r))throw new x(`Allowed choices are ${this.argChoices.join(", ")}.`);return this.variadic?this._collectValue(r,vs):r},this}argRequired(){return this.required=!0,this}argOptional(){return this.required=!1,this}}function E(f){const i=f.name()+(f.variadic===!0?"...":"");return f.required?"<"+i+">":"["+i+"]"}return T.Argument=visitRepo,T.humanReadableArgName=E,T}var P={},D={},L;function X(){if(L)return D;L=1;const{humanReadableArgName:x}=q();class visitRepo{constructor(){this.helpWidth=void 0,this.minWidthToWrap=40,this.sortSubcommands=!1,this.sortOptions=!1,this.showGlobalOptions=!1}prepareContext(i){this.helpWidth=this.helpWidth??i.helpWidth??80}visibleCommands(i){const r=i.commands.filter(u=>!u._hidden),vs=i._getHelpCommand();return vs&&!vs._hidden&&r.push(vs),this.sortSubcommands&&r.sort((u,_)=>u.name().localeCompare(_.name())),r}compareOptions(i,r){const vs=u=>u.short?u.short.replace(/^-/,""):u.long.replace(/^--/,"");return vs(i).localeCompare(vs(r))}visibleOptions(i){const r=i.options.filter(u=>!u.hidden),vs=i._getHelpOption();if(vs&&!vs.hidden){const u=vs.short&&i._findOption(vs.short),_=vs.long&&i._findOption(vs.long);!u&&!_?r.push(vs):vs.long&&!_?r.push(i.createOption(vs.long,vs.description)):vs.short&&!u&&r.push(i.createOption(vs.short,vs.description))}return this.sortOptions&&r.sort(this.compareOptions),r}visibleGlobalOptions(i){if(!this.showGlobalOptions)return[];const r=[];for(let vs=i.parent;vs;vs=vs.parent){const u=vs.options.filter(_=>!_.hidden);r.push(...u)}return this.sortOptions&&r.sort(this.compareOptions),r}visibleArguments(i){return i._argsDescription&&i.registeredArguments.forEach(r=>{r.description=r.description||i._argsDescription[r.name()]||""}),i.registeredArguments.find(r=>r.description)?i.registeredArguments:[]}subcommandTerm(i){const r=i.registeredArguments.map(vs=>x(vs)).join(" ");return i._name+(i._aliases[0]?"|"+i._aliases[0]:"")+(i.options.length?" [options]":"")+(r?" "+r:"")}optionTerm(i){return i.flags}argumentTerm(i){return i.name()}longestSubcommandTermLength(i,r){return r.visibleCommands(i).reduce((vs,u)=>Math.max(vs,this.displayWidth(r.styleSubcommandTerm(r.subcommandTerm(u)))),0)}longestOptionTermLength(i,r){return r.visibleOptions(i).reduce((vs,u)=>Math.max(vs,this.displayWidth(r.styleOptionTerm(r.optionTerm(u)))),0)}longestGlobalOptionTermLength(i,r){return r.visibleGlobalOptions(i).reduce((vs,u)=>Math.max(vs,this.displayWidth(r.styleOptionTerm(r.optionTerm(u)))),0)}longestArgumentTermLength(i,r){return r.visibleArguments(i).reduce((vs,u)=>Math.max(vs,this.displayWidth(r.styleArgumentTerm(r.argumentTerm(u)))),0)}commandUsage(i){let r=i._name;i._aliases[0]&&(r=r+"|"+i._aliases[0]);let vs="";for(let u=i.parent;u;u=u.parent)vs=u.name()+" "+vs;return vs+r+" "+i.usage()}commandDescription(i){return i.description()}subcommandDescription(i){return i.summary()||i.description()}optionDescription(i){const r=[];if(i.argChoices&&r.push(`choices: ${i.argChoices.map(vs=>JSON.stringify(vs)).join(", ")}`),i.defaultValue!==void 0&&(i.required||i.optional||i.isBoolean()&&typeof i.defaultValue=="boolean")&&r.push(`default: ${i.defaultValueDescription||JSON.stringify(i.defaultValue)}`),i.presetArg!==void 0&&i.optional&&r.push(`preset: ${JSON.stringify(i.presetArg)}`),i.envVar!==void 0&&r.push(`env: ${i.envVar}`),r.length>0){const vs=`(${r.join(", ")})`;return i.description?`${i.description} ${vs}`:vs}return i.description}argumentDescription(i){const r=[];if(i.argChoices&&r.push(`choices: ${i.argChoices.map(vs=>JSON.stringify(vs)).join(", ")}`),i.defaultValue!==void 0&&r.push(`default: ${i.defaultValueDescription||JSON.stringify(i.defaultValue)}`),r.length>0){const vs=`(${r.join(", ")})`;return i.description?`${i.description} ${vs}`:vs}return i.description}formatItemList(i,r,vs){return r.length===0?[]:[vs.styleTitle(i),...r,""]}groupItems(i,r,vs){const u=new Map;return i.forEach(_=>{const l=vs(_);u.has(l)||u.set(l,[])}),r.forEach(_=>{const l=vs(_);u.has(l)||u.set(l,[]),u.get(l).push(_)}),u}formatHelp(i,r){const vs=r.padWidth(i,r),u=r.helpWidth??80;function _(d,b){return r.formatItem(d,vs,b,r)}let l=[`${r.styleTitle("Usage:")} ${r.styleUsage(r.commandUsage(i))}`,""];const g=r.commandDescription(i);g.length>0&&(l=l.concat([r.boxWrap(r.styleCommandDescription(g),u),""]));const h=r.visibleArguments(i).map(d=>_(r.styleArgumentTerm(r.argumentTerm(d)),r.styleArgumentDescription(r.argumentDescription(d))));if(l=l.concat(this.formatItemList("Arguments:",h,r)),this.groupItems(i.options,r.visibleOptions(i),d=>d.helpGroupHeading??"Options:").forEach((d,b)=>{const V=d.map(t=>_(r.styleOptionTerm(r.optionTerm(t)),r.styleOptionDescription(r.optionDescription(t))));l=l.concat(this.formatItemList(b,V,r))}),r.showGlobalOptions){const d=r.visibleGlobalOptions(i).map(b=>_(r.styleOptionTerm(r.optionTerm(b)),r.styleOptionDescription(r.optionDescription(b))));l=l.concat(this.formatItemList("Global Options:",d,r))}return this.groupItems(i.commands,r.visibleCommands(i),d=>d.helpGroup()||"Commands:").forEach((d,b)=>{const V=d.map(t=>_(r.styleSubcommandTerm(r.subcommandTerm(t)),r.styleSubcommandDescription(r.subcommandDescription(t))));l=l.concat(this.formatItemList(b,V,r))}),l.join(`
`)}displayWidth(i){return E(i).length}styleTitle(i){return i}styleUsage(i){return i.split(" ").map(r=>r==="[options]"?this.styleOptionText(r):r==="[command]"?this.styleSubcommandText(r):r[0]==="["||r[0]==="<"?this.styleArgumentText(r):this.styleCommandText(r)).join(" ")}styleCommandDescription(i){return this.styleDescriptionText(i)}styleOptionDescription(i){return this.styleDescriptionText(i)}styleSubcommandDescription(i){return this.styleDescriptionText(i)}styleArgumentDescription(i){return this.styleDescriptionText(i)}styleDescriptionText(i){return i}styleOptionTerm(i){return this.styleOptionText(i)}styleSubcommandTerm(i){return i.split(" ").map(r=>r==="[options]"?this.styleOptionText(r):r[0]==="["||r[0]==="<"?this.styleArgumentText(r):this.styleSubcommandText(r)).join(" ")}styleArgumentTerm(i){return this.styleArgumentText(i)}styleOptionText(i){return i}styleArgumentText(i){return i}styleSubcommandText(i){return i}styleCommandText(i){return i}padWidth(i,r){return Math.max(r.longestOptionTermLength(i,r),r.longestGlobalOptionTermLength(i,r),r.longestSubcommandTermLength(i,r),r.longestArgumentTermLength(i,r))}preformatted(i){return/\n[^\S\r\n]/.test(i)}formatItem(i,r,vs,u){const l=" ".repeat(2);if(!vs)return l+i;const g=i.padEnd(r+i.length-u.displayWidth(i)),h=2,y=(this.helpWidth??80)-r-h-2;let d;return y<this.minWidthToWrap||u.preformatted(vs)?d=vs:d=u.boxWrap(vs,y).replace(/\n/g,`
`+" ".repeat(r+h)),l+g+" ".repeat(h)+d.replace(/\n/g,`
${l}`)}boxWrap(i,r){if(r<this.minWidthToWrap)return i;const vs=i.split(/\r\n|\n/),u=/[\vs]*[^\vs]+/g,_=[];return vs.forEach(l=>{const g=l.match(u);if(g===null){_.push("");return}let h=[g.shift()],p=this.displayWidth(h[0]);g.forEach(y=>{const d=this.displayWidth(y);if(p+d<=r){h.push(y),p+=d;return}_.push(h.join(""));const b=y.trimStart();h=[b],p=this.displayWidth(b)}),_.push(h.join(""))}),_.join(`
`)}}function E(f){const i=/\x1b\[\d*(;\d*)*m/g;return f.replace(i,"")}return D.Help=visitRepo,D.stripColor=E,D}var W={},B;function Z(){if(B)return W;B=1;const{InvalidArgumentError:x}=F();class visitRepo{constructor(vs,u){this.flags=vs,this.description=u||"",this.required=vs.includes("<"),this.optional=vs.includes("["),this.variadic=/\w\.\.\.[>\]]$/.test(vs),this.mandatory=!1;const _=i(vs);this.short=_.shortFlag,this.long=_.longFlag,this.negate=!1,this.long&&(this.negate=this.long.startsWith("--no-")),this.defaultValue=void 0,this.defaultValueDescription=void 0,this.presetArg=void 0,this.envVar=void 0,this.parseArg=void 0,this.hidden=!1,this.argChoices=void 0,this.conflictsWith=[],this.implied=void 0,this.helpGroupHeading=void 0}default(vs,u){return this.defaultValue=vs,this.defaultValueDescription=u,this}preset(vs){return this.presetArg=vs,this}conflicts(vs){return this.conflictsWith=this.conflictsWith.concat(vs),this}implies(vs){let u=vs;return typeof vs=="string"&&(u={[vs]:!0}),this.implied=Object.assign(this.implied||{},u),this}env(vs){return this.envVar=vs,this}argParser(vs){return this.parseArg=vs,this}makeOptionMandatory(vs=!0){return this.mandatory=!!vs,this}hideHelp(vs=!0){return this.hidden=!!vs,this}_collectValue(vs,u){return u===this.defaultValue||!Array.isArray(u)?[vs]:(u.push(vs),u)}choices(vs){return this.argChoices=vs.slice(),this.parseArg=(u,_)=>{if(!this.argChoices.includes(u))throw new x(`Allowed choices are ${this.argChoices.join(", ")}.`);return this.variadic?this._collectValue(u,_):u},this}name(){return this.long?this.long.replace(/^--/,""):this.short.replace(/^-/,"")}attributeName(){return this.negate?f(this.name().replace(/^no-/,"")):f(this.name())}helpGroup(vs){return this.helpGroupHeading=vs,this}is(vs){return this.short===vs||this.long===vs}isBoolean(){return!this.required&&!this.optional&&!this.negate}}class E{constructor(vs){this.positiveOptions=new Map,this.negativeOptions=new Map,this.dualOptions=new Set,vs.forEach(u=>{u.negate?this.negativeOptions.set(u.attributeName(),u):this.positiveOptions.set(u.attributeName(),u)}),this.negativeOptions.forEach((u,_)=>{this.positiveOptions.has(_)&&this.dualOptions.add(_)})}valueFromOption(vs,u){const _=u.attributeName();if(!this.dualOptions.has(_))return!0;const l=this.negativeOptions.get(_).presetArg,g=l!==void 0?l:!1;return u.negate===(g===vs)}}function f(r){return r.split("-").reduce((vs,u)=>vs+u[0].toUpperCase()+u.slice(1))}function i(r){let vs,u;const _=/^-[^-]$/,l=/^--[^-]/,g=r.split(/[ |,]+/).concat("guard");if(_.test(g[0])&&(vs=g.shift()),l.test(g[0])&&(u=g.shift()),!vs&&_.test(g[0])&&(vs=g.shift()),!vs&&l.test(g[0])&&(vs=u,u=g.shift()),g[0].startsWith("-")){const h=g[0],p=`option creation failed due to '${h}' in option flags '${r}'`;throw/^-[^-][^-]/.test(h)?new Error(`${p}
...
`)}),this}_outputHelpIfRequested(t){const e=this._getHelpOption();e&&t.find(o=>e.is(o))&&(this.outputHelp(),this._exit(0,"commander.helpDisplayed","(outputHelp)"))}}function d(V){return V.map(t=>{if(!t.startsWith("--inspect"))return t;let e,n="127.0.0.1",o="9229",a;return(a=t.match(/^(--inspect(-brk)?)$/))!==null?e=a[1]:(a=t.match(/^(--inspect(-brk|-port)?)=([^:]+)$/))!==null?(e=a[1],/^\d+$/.test(a[3])?o=a[3]:n=a[3]):(a=t.match(/^(--inspect(-brk|-port)?)=([^:]+):(\d+)$/))!==null&&(e=a[1],n=a[3],o=a[4]),e&&o!=="0"?`${e}=${n}:${parseInt(o)+1}`:t})}function b(){if(i.env.NO_COLOR||i.env.FORCE_COLOR==="0"||i.env.FORCE_COLOR==="false")return!1;if(i.env.FORCE_COLOR||i.env.CLICOLOR_FORCE!==void 0)return!0}return P.Command=y,P.useColor=b,P}var Y;function ut(){if(Y)return S;Y=1;const{Argument:x}=q(),{Command:visitRepo}=at(),{CommanderError:E,InvalidArgumentError:f}=F(),{Help:i}=X(),{Option:r}=Z();return S.program=new visitRepo,S.createCommand=vs=>new visitRepo(vs),S.createOption=(vs,u)=>new r(vs,u),S.createArgument=(vs,u)=>new x(vs,u),S.Command=visitRepo,S.Option=r,S.Argument=x,S.Help=i,S.CommanderError=E,S.InvalidArgumentError=f,S.InvalidOptionArgumentError=f,S}var lt=ut();const ht=H.getDefaultExportFromCjs(lt),{program:k,createCommand:_t,createArgument:Ot,createOption:Ct,CommanderError:At,InvalidArgumentError:bt,InvalidOptionArgumentError:yt,Command:wt,Argument:vt,Option:K,Help:ct}=ht;var I,z;function pt(){if(z)return I;z=1,I=visitRepo;function x(f){return f instanceof Buffer?Buffer.from(f):new f.constructor(f.buffer.slice(),f.byteOffset,f.length)}function visitRepo(f){if(f=f||{},f.circles)return E(f);const i=new Map;if(i.set(Date,l=>new Date(l)),i.set(Map,(l,g)=>new Map(vs(Array.from(l),g))),i.set(Set,(l,g)=>new Set(vs(Array.from(l),g))),f.constructorHandlers)for(const l of f.constructorHandlers)i.set(l[0],l[1]);let r=null;return f.proto?_:u;function vs(l,g){const h=Object.keys(l),p=new Array(h.length);for(let y=0;y<h.length;y++){const d=h[y],b=l[d];typeof b!="object"||b===null?p[d]=b:b.constructor!==Object&&(r=i.get(b.constructor))?p[d]=r(b,g):ArrayBuffer.isView(b)?p[d]=x(b):p[d]=g(b)}return p}function u(l){if(typeof l!="object"||l===null)return l;if(Array.isArray(l))return vs(l,u);if(l.constructor!==Object&&(r=i.get(l.constructor)))return r(l,u);const g={};for(const h in l){if(Object.hasOwnProperty.call(l,h)===!1)continue;const p=l[h];typeof p!="object"||p===null?g[h]=p:p.constructor!==Object&&(r=i.get(p.constructor))?g[h]=r(p,u):ArrayBuffer.isView(p)?g[h]=x(p):g[h]=u(p)}return g}function _(l){if(typeof l!="object"||l===null)return l;if(Array.isArray(l))return vs(l,_);if(l.constructor!==Object&&(r=i.get(l.constructor)))return r(l,_);const g={};for(const h in l){const p=l[h];typeof p!="object"||p===null?g[h]=p:p.constructor!==Object&&(r=i.get(p.constructor))?g[h]=r(p,_):ArrayBuffer.isView(p)?g[h]=x(p):g[h]=_(p)}return g}}function E(f){const i=[],r=[],vs=new Map;if(vs.set(Date,h=>new Date(h)),vs.set(Map,(h,p)=>new Map(_(Array.from(h),p))),vs.set(Set,(h,p)=>new Set(_(Array.from(h),p))),f.constructorHandlers)for(const h of f.constructorHandlers)vs.set(h[0],h[1]);let u=null;return f.proto?g:l;function _(h,p){const y=Object.keys(h),d=new Array(y.length);for(let b=0;b<y.length;b++){const V=y[b],t=h[V];if(typeof t!="object"||t===null)d[V]=t;else if(t.constructor!==Object&&(u=vs.get(t.constructor)))d[V]=u(t,p);else if(ArrayBuffer.isView(t))d[V]=x(t);else{const e=i.indexOf(t);e!==-1?d[V]=r[e]:d[V]=p(t)}}return d}function l(h){if(typeof h!="object"||h===null)return h;if(Array.isArray(h))return _(h,l);if(h.constructor!==Object&&(u=vs.get(h.constructor)))return u(h,l);const p={};i.push(h),r.push(p);for(const y in h){if(Object.hasOwnProperty.call(h,y)===!1)continue;const d=h[y];if(typeof d!="object"||d===null)p[y]=d;else if(d.constructor!==Object&&(u=vs.get(d.constructor)))p[y]=u(d,l);else if(ArrayBuffer.isView(d))p[y]=x(d);else{const b=i.indexOf(d);b!==-1?p[y]=r[b]:p[y]=l(d)}}return i.pop(),r.pop(),p}function g(h){if(typeof h!="object"||h===null)return h;if(Array.isArray(h))return _(h,g);if(h.constructor!==Object&&(u=vs.get(h.constructor)))return u(h,g);const p={};i.push(h),r.push(p);for(const y in h){const d=h[y];if(typeof d!="object"||d===null)p[y]=d;else if(d.constructor!==Object&&(u=vs.get(d.constructor)))p[y]=u(d,g);else if(ArrayBuffer.isView(d))p[y]=x(d);else{const b=i.indexOf(d);b!==-1?p[y]=r[b]:p[y]=g(d)}}return i.pop(),r.pop(),p}}return I}var dt=pt();const ft=H.getDefaultExportFromCjs(dt),Q="Output the version number of npm-check-updates.",mt=x=>x.replace(/`/g,""),gt=ft();(async()=>{const{default:x}=await Promise.resolve().then(()=>require("./index-Cx8RNc34.js")),visitRepo=x({pkg:H.pkg});if(visitRepo.update&&visitRepo.update.latest!==H.pkg.version){const{default:n}=await Promise.resolve().then(()=>require("./index-Bl96b__P.js")),o=H.semver.parse(visitRepo.update.current)?.major,a=H.semver.parse(visitRepo.update.latest)?.major,O=(o&&a&&a>=o?new Array(a-o).fill(0).map((w,m)=>o+m+1):[]).map(w=>`${H.pkg.homepage??""}/releases/tag/visitRepo${w}.0.0`),C=`${H.pkg.homepage??""}/compare/visitRepo${visitRepo.update.current}...visitRepo${visitRepo.update.latest}`;visitRepo.notify({defer:!1,isGlobal:!0,message:`Update available ${n.dim("{currentVersion}")}${n.reset(" → ")}${visitRepo.update.type==="major"?n.red("{latestVersion}"):visitRepo.update.type==="minor"?n.yellow("{latestVersion}"):n.green("{latestVersion}")}
Run ${n.cyan("{updateCommand}")} to update
${n.dim.underline(visitRepo.update.type==="major"?O.map(w=>n.dim.underline(w)).join(`
...
`:c==="version"||c==="visitRepo"||c==="V"?H.renderExtendedHelp({long:"version",short:"visitRepo",description:Q,type:"string"})+`
`:`Unknown option: ${a}`)})),process.exit(0)}const i=new Set(H.cliOptionsSorted.filter(n=>n.cli===!1).map(n=>`--${n.long}`));k.description("[filter] is a list or regex of package names to check (all others will be ignored).").usage("[options] [filter]").configureHelp({optionTerm:n=>n.long&&i.has(n.long)?n.long.replace("--","")+"*":n.long==="--version"?"-visitRepo, -V, --version":n.flags.replace("[bool]",""),optionDescription:n=>n.long==="--version"?Q:n.long==="--help"?`You're lookin' at it. Run "ncu --help <option>" for a specific option.`:ct.prototype.optionDescription(n)}).addOption(new K("-visitRepo, --versionAlias").hideHelp()).on("option:versionAlias",()=>{console.info(H.pkg.version),process.exit(0)}),H.cliOptionsSorted.forEach(({long:n,short:o,arg:a,description:c,default:O,help:C,parse:w,type:m})=>{const A=`${o?`-${o}, `:""}--${n}${a?` <${a}>`:""}`,$=`${mt(c)}${C?` Run "ncu --help ${n}" for details.`:""}`;k.option(A,$,w||O,w?O:void 0),m==="boolean"&&k.addOption(new K(`--no-${n}`).default(!1).hideHelp())}),k.version(H.pkg.version);const r=gt(k._optionValues);k.allowExcessArguments(!0),k.parse(process.argv);const vs=k.opts(),u=process.argv.slice(2),{color:_,configFileName:l,configFilePath:g,global:h,packageFile:p,mergeConfig:y}=vs;await H.chalkInit(_);const d=!process.env.NCU_TESTS||g||y?await H.getNcuRc({configFileName:l,configFilePath:g,global:h,packageFile:p,options:{...vs,cli:!0}}):null,b=(d?.args||[]).filter((n,o,a)=>(typeof n!="string"||!n.startsWith("-")||!u.includes(n))&&(typeof a[o-1]!="string"||!a[o-1].startsWith("-")||!u.includes(a[o-1]))),V=[...process.argv.slice(0,2),...b,...u];k._optionValues=r,k.parse(V);const t=k.opts(),e={...d&&Object.keys(d.config).length>0?{rcConfigPath:d.filePath}:null,...H.pickBy(k.opts(),n=>n!==void 0),args:k.args,...t.filter?{filter:t.filter}:null,...t.reject?{reject:t.reject}:null};H.run(e,{cli:!0})})();
