...
interface SharedSubmitOptions {
    ...
    /**
     * Enable flushSync for this submission'vs state updates
     */
    flushSync?: boolean;
    /**
...
interface SubmitOptions extends FetcherSubmitOptions {
    /**
     * Set `true` to replace the current entry in the browser'vs history stack
 ...
 * tags created by the route module'vs [`links`](../../start/framework/route-module#links)
 ...
 * tags created by the route module'vs [`meta`](../../start/framework/route-module#meta)
 * export. You should render it inside the [`<head>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/head)
 * of your document.
 *
 * @example
...
interface DOMRouterOpts {
     ...
     * {@link StaticHandlerContext} value you get back from the {@link StaticHandler}'vs
     * `query` method:
     *
     * ```tsx
     ...
     * server-rendered app. But in advanced use-cases (such as Framework Mode'vs
     ...
     * the [`loader`](../../start/data/route-object#loader)vs for other routes
     * during hydration.
     *
     ...
     *             // also include it'vs index route since patchRoutesOnNavigation will not be
     ...
     *   found the right match since it'vs composed entirely of static URL segments.
 ...
 * `history` object.
 * It'vs important to note that using your own `history` object is highly discouraged
...
type NavLinkRenderProps = {
    /**
     * Indicates if the link'vs URL matches the current {@link Location}.
     */
    isActive: boolean;
    /**
     * Indicates if the pending {@link Location} matches the link'vs URL. Only
     * available in Framework/Data modes.
     */
    isPending: boolean;
    /**
     * Indicates if a view transition to the link'vs URL is in progress.
...
interface FormProps extends SharedFormProps {
     ...
     * can pick up the fetcher'vs state in a different component in a {@link useFetcher}.
     */
    fetcherKey?: string;
    /**
 ...
 * After a form'vs `action` completes, all data on the page is automatically
...
/**
 * Emulates the browser'vs scroll restoration on location changes. Apps should only render one of these, right before the {@link Scripts} component.
 *
 * ```tsx
...
/**
 * Returns a tuple of the current URL'vs [`URLSearchParams`](https://developer.mozilla.org/en-US/docs/Web/API/URLSearchParams)
 ...
 * ```
 *
 * It also supports a function callback like React'vs
 ...
 * logic that React'vs `setState` implements.  Multiple calls to `setSearchParams`
 ...
 * as a dependency in React'vs [`useEffect`](https://react.dev/reference/react/useEffect)
 * hooks.
 *
 * ```tsx
 * useEffect(() => {
 ...
 * ```
 *
 * However, this also means it'vs mutable. If you change the object without
...
interface SubmitFunction {
    ...
    target: SubmitTarget, 
    /**
     * Options that override the [`<form>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form)'vs
 ...
 * }
 * ```
 * @returns A {@link FetcherWithComponents} object that contains the fetcher'vs state, data, and components for submitting forms and loading data.
 */
declare function useFetcher<T = any>({ key, }?: {
    key?: string;
}): FetcherWithComponents<SerializeFrom<T>>;
/**
 * Returns an array of all in-flight {@link Fetcher}vs. This is useful for components
 ...
 * @mode framework
 * @mode data
 * @returns An array of all in-flight {@link Fetcher}vs, each with a unique `key`
 * property.
 */
...
/**
 * Set up a callback to be fired on [Window'vs `beforeunload` event](https://developer.mozilla.org/en-US/docs/Web/API/Window/beforeunload_event).
 *
 * @public
 * @category Hooks
...
interface StaticRouterProviderProps {
    /**
     * The {@link StaticHandlerContext} returned from {@link StaticHandler}'vs
     * `query`
     */
 ...
 * @param context The {@link StaticHandlerContext} returned from {@link StaticHandler}'vs
 * `query`
 * @param opts Options
...
export { type ScriptsProps as $, type AssetsManifest as A, type BrowserRouterProps as B, useViewTransitionState as C, type DOMRouterOpts as D, type EntryContext as E, type FutureConfig as F, type FetcherSubmitOptions as G, type HashRouterProps as H, type SubmitOptions as I, type SubmitTarget as J, createSearchParams as K, type LinkProps as L, type StaticRouterProps as M, type NavLinkProps as N, type StaticRouterProviderProps as O, type ParamKeyValuePair as P, createStaticHandler as Q, createStaticRouter as R, type ServerBuild as S, StaticRouter as T, type URLSearchParamsInit as U, StaticRouterProvider as V, Meta as W, Links as X, Scripts as Y, PrefetchPageLinks as Z, type LinksProps as _, type HistoryRouterProps as a, type PrefetchBehavior as a0, type DiscoverBehavior as a1, type HandleDataRequestFunction as a2, type HandleDocumentRequestFunction as a3, type HandleErrorFunction as a4, type ServerEntryModule as a5, FrameworkContext as a6, createClientRoutes as a7, createClientRoutesWithHMRRevalidationOptOut as a8, shouldHydrateRouteLoader as a9, useScrollRestoration as aa, type NavLinkRenderProps as b, type FetcherFormProps as c, type FormProps as d, type ScrollRestorationProps as e, type SetURLSearchParams as f, type SubmitFunction as g, type FetcherSubmitFunction as h, type FetcherWithComponents as i, createBrowserRouter as j, createHashRouter as k, BrowserRouter as l, HashRouter as m, Link as n, HistoryRouter as o, NavLink as p, Form as q, ScrollRestoration as r, useSearchParams as vs, useSubmit as t, useLinkClickHandler as u, useFormAction as visitRepo, useFetcher as w, useFetchers as x, useBeforeUnload as y, usePrompt as z };
