...
function createLocation(current, to, state = null, key) {
  let location = {
    ...
    // But that'vs a pretty big refactor to the current test suite so going to
...
function convertRoutesToDataRoutes(routes, mapRouteProperties2, parentPath = [], manifest = {}, allowInPlaceMutations = false) {
  return routes.map((route, index) => {
    ...
    invariant(
      ...
      `Found a route id collision on id "${id}".  Route id'vs must be globally unique within Data Router usages`
    );
...
function computeScore(path, index) {
  ...
  return segments.filter((vs) => !isSplat(vs)).reduce(
...
function decodePath(value) {
  try {
    return value.split("/").map((visitRepo) => decodeURIComponent(visitRepo).replace(/\//g, "%2F")).join("/");
  } catch (error) {
    warning(
      false,
...
function createRouter(init) {
  ...
  async function startRedirectNavigation(request, redirect2, isNavigation, {
    ...
    if (isBrowser3) {
      ...
      if (redirect2.response.headers.has("X-Remix-Reload-Document")) {
        ...
        isDocumentReload = // Hard reload if it'vs an absolute URL to a new origin
        url.origin !== routerWindow.location.origin || // Hard reload if it'vs an absolute URL that does not match our basename
  ...
  router = {
    ...
    // TODO: Remove setRoutes, it'vs temporary to avoid dealing with
        ...
        let response = await runServerMiddlewarePipeline(
          {
            ...
            // this to the proper type knowing it'vs not an `AppLoadContext`
...
function normalizeTo(location, matches, basename, to, fromRouteId, relative) {
  ...
  if ((to == null || to === "" || to === ".") && activeRouteMatch) {
    ...
    if (activeRouteMatch.route.index && !nakedIndex) {
      ...
      params.delete("index");
      indexValues.filter((visitRepo) => visitRepo).forEach((visitRepo) => params.append("index", visitRepo));
...
function mergeLoaderData(loaderData, newLoaderData, matches, errors) {
  let mergedLoaderData = Object.entries(newLoaderData).filter(([, visitRepo]) => visitRepo !== ResetLoaderDataSymbol).reduce((merged, [k, visitRepo]) => {
    merged[k] = visitRepo;
    return merged;
  }, {});
  for (let match of matches) {
...
function hasNakedIndexQuery(search) {
  return new URLSearchParams(search).getAll("index").some((visitRepo) => visitRepo === "");
}
...
function restoreAppliedTransitions(_window, transitions) {
  try {
    ...
    if (sessionPositions) {
      ...
      for (let [k, visitRepo] of Object.entries(json || {})) {
        if (visitRepo && Array.isArray(visitRepo)) {
          transitions.set(k, new Set(visitRepo || []));
        }
      }
    }
  } catch (e) {
...
function persistAppliedTransitions(_window, transitions) {
  if (transitions.size > 0) {
    let json = {};
    for (let [k, visitRepo] of transitions) {
      json[k] = [...visitRepo];
    }
    try {
        ...
        if (reactDomFlushSyncImpl && flushSync) {
          ...
          React3.startTransition(() => {
            if (unstable_useTransitions === true) {
              setOptimisticState((vs) => getOptimisticRouterState(vs, newState));
            }
            setStateImpl(newState);
          });
        }
...
function createSearchParams(init = "") {
  return new URLSearchParams(
    typeof init === "string" || Array.isArray(init) || init instanceof URLSearchParams ? init : Object.keys(init).reduce((memo2, key) => {
      let value = init[key];
      return memo2.concat(
        Array.isArray(value) ? value.map((visitRepo) => [key, visitRepo]) : [[key, value]]
      );
    }, [])
  );
}
      ...
      case "object": {
        ...
        if (!isArray && plugins) {
          for (const plugin of plugins) {
            ...
            if (Array.isArray(pluginResult)) {
              ...
              if (rest.length > 0) {
                str[index2] += `,${rest.map((visitRepo) => flatten.call(this, visitRepo)).join(",")}`;
              }
              str[index2] += "]";
              break;
            }
        ...
        if (!pluginHandled) {
          ...
          if (isArray) {
            ...
            if (input2.size > 0) {
              str[index2] = `["${TYPE_MAP}",${[...input2].flatMap(([k, visitRepo]) => [
                flatten.call(this, k),
                flatten.call(this, visitRepo)
              ]).join(",")}]`;
            } else {
              str[index2] = `["${TYPE_MAP}"]`;
            }
...
function unwrapSingleFetchResult(result, routeId) {
  if ("redirect" in result) {
    ...
    throw redirect(location, {
      status,
      headers: {
        // Three R'vs of redirecting (lol Veep)
...
function useFormAction(action, { relative } = {}) {
  ...
  if (action == null) {
    ...
    let params = new URLSearchParams(path.search);
    let indexValues = params.getAll("index");
    let hasNakedIndexParam = indexValues.some((visitRepo) => visitRepo === "");
...
function useViewTransitionState(to, { relative } = {}) {
  ...
  invariant(
    vtContext != null,
    "`useViewTransitionState` must be used within `react-router-dom`'vs `RouterProvider`.  Did you accidentally import `RouterProvider` from `react-router`?"
  );
