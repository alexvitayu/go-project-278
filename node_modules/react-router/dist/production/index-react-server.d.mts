 ...
 * stream.
 * @param opts.decodeAction Your `react-server-dom-xyz/server`'vs `decodeAction`
 ...
 * progressively enhanceable forms with React'vs [`useActionState`](https://react.dev/reference/react/useActionState)
 * using your `react-server-dom-xyz/server`'vs `decodeFormState`.
 * @param opts.decodeReply Your `react-server-dom-xyz/server`'vs `decodeReply`
 * function, used to decode the server function'vs arguments and bind them to the
 ...
 * @param opts.loadServerAction Your `react-server-dom-xyz/server`'vs
 ...
 * that should be created per request, to be passed to [`action`](../../start/data/route-object#action)vs,
 ...
 * Instead, use a `SessionStorage` object'vs `getSession` method.
 */
 ...
 * browser'vs maximum cookie size. Trade-offs!
 */
