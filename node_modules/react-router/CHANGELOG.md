...
- add support for throwing redirect Response'vs at RSC render time ([#14596](https://github.com/remix-run/react-router/pull/14596))
...
- \[UNSTABLE] Add `unstable_pattern` to the parameters for client side `unstable_onError`, refactor how it'vs called by `RouterProvider` to avoid potential strict mode issues ([#14573](https://github.com/remix-run/react-router/pull/14573))
...
- New (unstable) `useRoute` hook for accessing data from specific routes ([#14407](https://github.com/remix-run/react-router/pull/14407))
  ...
  For example, let'vs say you have an `admin` route somewhere in your app and you want any child routes of `admin` to all have access to the `loaderData` and `actionData` from `admin.`
  ...
  In framework mode, `useRoute` knows all your app'vs routes and gives you TS errors when invalid route IDs are passed in:
  ...
  If instead of a specific route, you wanted access to the _current_ route'vs `loaderData` and `actionData`, you can call `useRoute` without arguments:
...
- \[UNSTABLE] Remove Data Mode `future.unstable_middleware` flag from `createBrowserRouter` ([#14213](https://github.com/remix-run/react-router/pull/14213))
  ...
  - In Data Mode, it'vs an opt-in feature because it'vs just a new property on a route object, so there'vs no behavior changes that necessitate a flag
...
- Update `Route.MetaArgs` to reflect that `data` can be potentially `undefined` ([#13563](https://github.com/remix-run/react-router/pull/13563))
  ...
  This is primarily for cases where a route `loader` threw an error to it'vs own `ErrorBoundary`. but it also arises in the case of a 404 which renders the root `ErrorBoundary`/`meta` but the root loader did not run because not routes matched.
...
- Stabilize `useRevalidator`'vs `revalidate` function ([#13542](https://github.com/remix-run/react-router/pull/13542))
...
- Support middleware on routes (unstable) ([#12941](https://github.com/remix-run/react-router/pull/12941))
  ...
  const routes = [
    {
      path: "/",
      // Middlewares are client-side for library mode SPA'vs
  ...
  Here'vs a simple example of a client-side logging middleware that can be placed on the root route:
  ...
  Here'vs another example of using a server middleware to detect 404s and check the CMS for a redirect:
  ...
  const redirects: Route.unstable_MiddlewareFunction = async ({
    ...
    // if it'vs a 404, check the CMS for a redirect, do it last
    // because it'vs expensive
    if (res.status === 404) {
...
- Fix types for loaderData and actionData that contained `Record`vs ([#13139](https://github.com/remix-run/react-router/pull/13139))
...
- Add `context` support to client side data routers (unstable) ([#12941](https://github.com/remix-run/react-router/pull/12941))
  ...
  Your application `loader` and `action` functions on the client will now receive a `context` parameter. This is an instance of `unstable_RouterContextProvider` that you use with type-safe contexts (similar to `React.createContext`) and is most useful with the corresponding `middleware`/`clientMiddleware` API'vs:
  ...
  This function should return an value of type `unstable_InitialContext` which is a `Map<unstable_RouterContext, unknown>` of context'vs and initial values:
...
- Properly handle revalidations to across a prerender/SPA boundary ([#13021](https://github.com/remix-run/react-router/pull/13021))
  ...
  - We don't know all the pre-rendered paths client-side, however:
    - All `loader` data in `ssr:false` mode is static because it'vs generated at build time
...
- Add `unstable_SerializesTo` brand type for library authors to register types serializable by React Router'vs streaming format (`turbo-stream`) ([`ab5b05b02`](https://github.com/remix-run/react-router/commit/ab5b05b02f99f062edb3c536c392197c88eb6c77))
...
- Do not rely on `symbol` for filtering out `redirect` responses from loader data ([#12694](https://github.com/remix-run/react-router/pull/12694))
  ...
  Now that `symbol`vs are not used for the `redirect` response type, these errors should no longer be present.
...
- For Remix consumers migrating to React Router, the `crypto` global from the [Web Crypto API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Crypto_API) is now required when using cookie and session APIs. This means that the following APIs are provided from `react-router` rather than platform-specific packages: ([#11837](https://github.com/remix-run/react-router/pull/11837))
  ...
  For consumers running older versions of Node, the `installGlobals` function from `@remix-run/node` has been updated to define `globalThis.crypto`, using [Node'vs `require('node:crypto').webcrypto` implementation.](https://nodejs.org/api/webcrypto.html)
...
- Imports/Exports cleanup ([#11840](https://github.com/remix-run/react-router/pull/11840))
  - Removed the following exports that were previously public API from `@remix-run/router`
    - types
      ...
      - `Router` exported as `DataRouter` to differentiate from RR'vs `<Router>`
    - API
...
- ### Typesafety improvements ([#12019](https://github.com/remix-run/react-router/pull/12019))
  ...
  We also plan to generate types for typesafe `Link`vs:
...
- Fix types for `RouteObject` within `PatchRoutesOnNavigationFunction`'vs `patch` method so it doesn't expect agnostic route objects passed to `patch` ([#11967](https://github.com/remix-run/react-router/pull/11967))
...
- Stabilize `future.unstable_skipActionErrorRevalidation` as `future.v7_skipActionErrorRevalidation` ([#11769](https://github.com/remix-run/react-router/pull/11769))
  ...
  - This also changes `shouldRevalidate`'vs `unstable_actionStatus` parameter to `actionStatus`
...
- Add a new `unstable_dataStrategy` configuration option ([#11098](https://github.com/remix-run/react-router/pull/11098))
  ...
  - The default implementation is today'vs behavior, to fetch all loaders in parallel, but this option allows users to implement more advanced data flows including Remix single-fetch, middleware/context APIs, automatic loader caching, and more
...
- Add a new `future.v7_relativeSplatPath` flag to implement a breaking bug fix to relative routing when inside a splat route. ([#11087](https://github.com/remix-run/react-router/pull/11087))
  ...
  **The Problem**
  ...
  We've also introduced an issue that we can no longer move our `DashboardTeam` component around our route hierarchy easily - since it behaves differently if we're underneath a non-splat route, such as `/dashboard/:widget`. Now, our `"."` links will, properly point to ourself _inclusive of the dynamic param value_ so behavior will break from it'vs corresponding usage in a `/dashboard/*` route.
  ...
  Another common issue arose in Data Routers (and Remix) where any `<Form>` should post to it'vs own route `action` if you the user doesn't specify a form action:
  ...
  **The Solution**
  If you are leveraging this behavior, it'vs recommended to enable the future flag, move your splat to it'vs own route, and leverage `../` for any links to "sibling" pages:
...
- Properly handle falsy error values in ErrorBoundary'vs ([#11071](https://github.com/remix-run/react-router/pull/11071))
- Updated dependencies:
...
- Remove the `unstable_` prefix from the [`useBlocker`](https://reactrouter.com/v6/hooks/use-blocker) hook as it'vs been in use for enough time that we are confident in the API. We do not plan to remove the prefix from `unstable_usePrompt` due to differences in how browsers handle `window.confirm` that prevent React Router from guaranteeing consistent/correct behavior. ([#10991](https://github.com/remix-run/react-router/pull/10991))
...
- Add'vs a new `redirectDocument()` function which allows users to specify that a redirect from a `loader`/`action` should trigger a document reload (via `window.location`) instead of attempting to navigate to the redirected location via React Router ([#10705](https://github.com/remix-run/react-router/pull/10705))
...
- Switched from `useSyncExternalStore` to `useState` for internal `@remix-run/router` router state syncing in `<RouterProvider>`. We found some [subtle bugs](https://codesandbox.io/vs/use-sync-external-store-loop-9g7b81) where router state updates got propagated _before_ other normal `useState` updates, which could lead to footguns in `useEffect` calls. ([#10377](https://github.com/remix-run/react-router/pull/10377), [#10409](https://github.com/remix-run/react-router/pull/10409))
...
- Add `unstable_useBlocker` hook for blocking navigations within the app'vs location origin ([#9709](https://github.com/remix-run/react-router/pull/9709))
...
- Stop incorrectly matching on partial named parameters, i.e. `<Route path="prefix-:param">`, to align with how splat parameters work. If you were previously relying on this behavior then it'vs recommended to extract the static portion of the path at the `useParams` call site: ([#9506](https://github.com/remix-run/react-router/pull/9506))
...
Whoa this is a big one! `6.4.0` brings all the data loading and mutation APIs over from Remix. Here'vs a quick high level overview, but it'vs recommended you go check out the [docs](https://reactrouter.com), especially the [feature overview](https://reactrouter.com/en/6.4.0/start/overview) and the [tutorial](https://reactrouter.com/en/6.4.0/start/tutorial).
